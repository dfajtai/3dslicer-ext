...
class GenericSpecimen:
    ...
    def _customize_workplace(self):
        ...
        # (or any other widget) has already cached a pointer to the current
        # default vtkMRMLSegmentEditorNode, calling AddDefaultNode() again with
        # a brand-new instance drops the scene's reference to the old one; once
        # its refcount hits zero the C++ object is freed, leaving that cached
        # pointer dangling -> native crash with no Python traceback the next
        # time it's touched (e.g. when switching to the Segment Editor module).
        # So: create it once, and on every subsequent call just re-apply the
        # config to the SAME node instance.
