...
__database_csv_path__ = os.path.join(current_nas_path,"Projects","mycotoxin","etc","database.csv")
__preseg_csv_path__ = os.path.join(current_nas_path,"Projects","mycotoxin","etc","img_paths.csv")
__study_dir__ =  os.path.join(current_nas_path,"Projects","mycotoxin","segmentation")
...
class SegmentPet(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.title = "SegmentPet"  # TODO: make this more human readable by adding spaces
...
This is a simple module for PET segmentation.
See more information in <a href="https://github.com/organization/projectname#SegmentPet">module documentation</a>.
"""
...
def registerSampleData():
  ...
  print("Sample for SegmentPet not implemented")
...
class SegmentPetWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
  ...
  def setup(self):
    ...
    uiWidget = slicer.util.loadUI(self.resourcePath('UI/SegmentPet.ui'))
...
class SegmentPetLogic(ScriptedLoadableModuleLogic):
...
class SegmentPetTest(ScriptedLoadableModuleTest):
  def setUp(self):
...
class Specimen():
  def __init__(self, ID, dbDictList, presegDictList, study_dir):
    ...
    self.T1_path = fix_path(str(self.preseg_paths["t1"]).replace(2*os.sep,os.sep))
    self.PET_path = fix_path(str(self.preseg_paths["pet"]).replace(2*os.sep,os.sep))
  ...
  def slicer_out_dir(self):
    return os.path.join(self.study_dir,self.ID,"pet")
  ...
  def load(self):
    ...
    # load PET
    PET_node = slicer.util.loadVolume(self.PET_path)
    self.node_dict[self.PET_path] = PET_node
    ...
    if os.path.exists(self.segment_path):
      ...
      segmentationNode.SetReferenceImageGeometryParameterFromVolumeNode(self.node_dict[self.PET_path])
      ...
      mask_node = slicer.util.loadLabelVolume(self.mask_path) #load segment mask      
      ...
      segment_m_img = slicer.modules.segmentations.logic().CreateOrientedImageDataFromVolumeNode(mask_node)
      
      segmentationNode.AddSegmentFromBinaryLabelmapRepresentation(segment_m_img,"mask", [.8,.3,.7])
    ...
    slicer.util.setSliceViewerLayers(background=T1_node,foreground=PET_node,foregroundOpacity=0.5)
    pet_node_disp = PET_node.GetDisplayNode()
    pet_node_disp.SetAndObserveColorNodeID('vtkMRMLPETProceduralColorNodePET-Rainbow2')
    pet_node_disp.SetThreshold(0,50)
    pet_node_disp.ApplyThresholdOn()
    pet_node_disp.SetWindowLevel(.5,10)

    slicer.util.setSliceViewerLayers(background=T1_node)
    slicer.util.setSliceViewerLayers(foreground=PET_node)

    # T1 = slicer.modules.SegmentPetWidget.logic.active_specimen.node_dict[slicer.modules.SegmentPetWidget.logic.active_specimen.T1_path]
    # PET = slicer.modules.SegmentPetWidget.logic.active_specimen.node_dict[slicer.modules.SegmentPetWidget.logic.active_specimen.PET_path
...
def batch_exporter():
    ...
    import os
    if slicer.modules.SegmentPetWidget.logic.hasActiveSpecimen:
        print("Please close active specimen!")
        return
    
    slicer.modules.SegmentPetWidget.onBtnInitializeStudy()
    for sid, specimen in slicer.modules.SegmentPetWidget.logic.Specimens.items():
        if not specimen.db_info["done"]=='1':
            continue
        #open specimen
        slicer.modules.SegmentPetWidget.logic.load_specimen(sid)

        segmentation_node = slicer.mrmlScene.GetNodesByClass("vtkMRMLSegmentationNode").GetItemAsObject(0)
        accepted = ["mask"]

        volume = specimen.node_dict[specimen.PET_path]
        ...
        #close specimen
        slicer.modules.SegmentPetWidget.logic.close_active_specimen(True)
