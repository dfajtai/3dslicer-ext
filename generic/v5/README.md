...
### `segment_editor` - a Segment Editor widget alapbeállításai

Ez a Segment Editor node-ot állítja be specimen betöltéskor (eddig ez
hardcode volt: mindig `OverwriteNone` / "allow overlap"). Minden mező
opcionális.
...
"segment_editor": {
  ...
  "active_effect": "Paint",    // opcionális: ez az effect legyen aktív, amint megnyílik a Segment Editor
...
`editorNode.GetAttribute("Paint,BrushSphere")`-rel nézd meg a tényleges
kulcsot és értéket, és tedd be az `attributes` blokkba - az felülír mindent
fölötte.
