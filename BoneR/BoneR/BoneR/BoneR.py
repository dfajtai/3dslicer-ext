...
current_data_path = "/nas/medicopus_share/Projects/ANIMALS/ERIK/batch01/"
...
__study_dir__ =  os.path.join(current_data_path,"manual_segmentation")
...
def showCtkInfoWithImage(parent=None, title="Welcome!", iconPath=None):
  # Create CTK message box
  msgBox = ctk.ctkMessageBox(parent if parent is not None else slicer.util.mainWindow())
  ...
  # If you want to show an image above the text
  if iconPath and os.path.exists(iconPath):
    # Create a QLabel with the image
    pixmap = qt.QPixmap(iconPath).scaled(320, 320, qt.Qt.KeepAspectRatio, qt.Qt.SmoothTransformation)
    iconLabel = qt.QLabel()
    iconLabel.setPixmap(pixmap)
    # Insert image label into message box layout
    layout = msgBox.layout()
    layout.addWidget(iconLabel, 0, 0, 1, layout.columnCount(), qt.Qt.AlignHCenter)
    ...
    if mainWin:
      mainRect = mainWin.geometry
      msgBoxRect = msgBox.frameGeometry
      centerPoint = mainRect.center()
      msgBoxRect.moveCenter(centerPoint)
      msgBox.move(msgBoxRect.topLeft())


    
def showBoneR():
  iconPath = os.path.join(os.path.dirname(__file__), "Resources", "Icons", "BoneR.png")
  showCtkInfoWithImage(title="BoneR", iconPath=iconPath)
  


class BoneR(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.title = "BoneR"  # TODO: make this more human readable by adding spaces
    ...
    This is a simple module for bone segmentation.
    See more information in <a href="https://github.com/organization/projectname#BoneR">module documentation</a>.
    """
...
def registerSampleData():
  ...
  print("Sample for BoneR not implemented")
...
class BoneRWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
  ...
  def setup(self):
    ...
    uiWidget = slicer.util.loadUI(self.resourcePath('UI/BoneR.ui'))
...
class BoneRLogic(ScriptedLoadableModuleLogic):
...
class BoneRTest(ScriptedLoadableModuleTest):
  def setUp(self):
...
class Specimen():
  def __init__(self, ID, dbDictList, presegDictList, study_dir):
    ...
    self.bg_path = self.get_img_path("CT")
    self.mask_path = self.get_img_path("bone_labels")
  ...
  def customize_workplace(self):
    ...
    # set opacity
    slicer.util.setSliceViewerLayers(labelOpacity=0)
...
def batch_exporter():
    ...
    import os
    if slicer.modules.BoneRWidget.logic.hasActiveSpecimen:
        print("Please close active specimen!")
        return
    
    slicer.modules.BoneRWidget.onBtnInitializeStudy()
    for sid, specimen in slicer.modules.BoneRWidget.logic.Specimens.items():
        if not specimen.db_info["done"]=='1':
            continue
        #open specimen
        slicer.modules.BoneRWidget.logic.load_specimen(sid,load_bg = False,volume_rendering = False)
        ...
        #close specimen
        slicer.modules.BoneRWidget.logic.close_active_specimen(True)
        qt.QApplication.processEvents()
